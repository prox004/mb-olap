...
import { Search, TrendingUp, TrendingDown, Inbox, ShieldAlert, Award, Package, Percent } from "lucide-react";
...
export default function CategoryInsights() {
  ...
  const [data, setData] = useState<{ best_selling: CategoryStat[]; lowest_selling: CategoryStat[]; all_categories: CategoryStat[] } | null>(null);
  ...
  useEffect(() => {
    async function fetchCategoryData() {
      ...
      try {
        ...
        const res = await fetch(`http://127.0.0.1:8000/api/v1/analytics/categories?${queryParams.toString()}`);
        if (res.ok) {
  ...
  useEffect(() => {
    async function fetchTopProducts() {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/v1/analytics/categories/top-products?category_name=${encodeURIComponent(selectedCategory.category)}`);
        if (res.ok) {
          const result = await res.json();
  ...
  if (error) {
    return (
      <div className="glass-card p-6 flex flex-col items-center justify-center h-48">
        ...
        <span className="text-sm font-semibold text-[#F4F4F5]">Failed to load category analytics data</span>
      </div>
    );
  }
  ...
  const filteredCategories = data?.all_categories.filter((c) =>
          ...
          {searchQuery.trim().length > 0 && (
            <div className="absolute left-0 right-0 mt-1 border border-[rgba(255,255,255,0.15)] rounded-lg max-h-60 overflow-y-auto z-50 shadow-2xl" style={{ backgroundColor: "#18181B" }}>
              {filteredCategories.length > 0 ? (
                ...
                <div className="text-xs text-[#A1A1AA] p-4 text-center" style={{ backgroundColor: "#18181B", color: "#A1A1AA" }}>No categories matched.</div>
              )}
            </div>
          )}
        </div>
          ...
          <div className="glass-card p-6 flex flex-col justify-between min-h-[350px]">
            ...
            <div className="space-y-3.5 mt-6">
              <div className="flex justify-between border-b border-white/15 pb-2">
                ...
                <span className="text-sm font-bold text-white font-mono">{formatCurrency(selectedCategory.sales)}</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Gross Profit share</span>
                <span className="text-sm font-bold text-emerald-400 font-mono">{formatCurrency(selectedCategory.profit)}</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Gross Margin %</span>
                <span className="text-sm font-bold text-purple-300 font-mono">{(selectedCategory.margin_pct || 0).toFixed(2)}%</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Active SKU count</span>
                <span className="text-sm font-bold text-indigo-300 font-mono">{selectedCategory.sku_count || 0} Products</span>
              </div>
              <div className="flex justify-between pb-1">
                <span className="text-sm text-[#A1A1AA]">Closing inventory stock</span>
                <span className="text-sm font-bold text-blue-300 font-mono">{selectedCategory.stock_qty.toLocaleString()} Units</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6 flex flex-col justify-between min-h-[350px]">
            ...
            <div className="space-y-3.5 mt-6">
              <div className="flex justify-between border-b border-white/15 pb-2">
                ...
                <span className="text-sm font-bold text-emerald-500 font-mono">{(selectedCategory.receive_qty || 0).toLocaleString()} Units</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Returned Goods Volume</span>
                <span className="text-sm font-bold text-rose-500 font-mono">{(selectedCategory.return_qty || 0).toLocaleString()} Units</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Category Return Rate %</span>
                <span className={`text-sm font-bold font-mono ${selectedCategory.return_rate_pct > 10 ? 'text-rose-400' : 'text-emerald-400'}`}>
              ...
              <div className="flex justify-between border-b border-white/15 pb-2">
                ...
                <span className="text-sm font-bold text-white font-mono">
                  {formatCurrency((selectedCategory.sales || 0) / (selectedCategory.sku_count || 1))}
                </span>
              </div>
              <div className="flex justify-between pb-1">
                <span className="text-sm text-[#A1A1AA]">Profit Margin Grade</span>
                <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${selectedCategory.margin_pct > 12 ? 'bg-emerald-900/40 text-emerald-300 border border-emerald-700/40' : 'bg-amber-900/40 text-amber-300 border border-amber-700/40'
                  }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 flex flex-col justify-between h-[440px]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Top 10 Best Selling Categories</h3>
              ...
              <table className="w-full text-left text-[11px] font-mono border-collapse">
                <thead>
                  <tr className="border-b border-[rgba(255,255,255,0.08)] bg-white/5 sticky top-0 bg-[#121217]">
                    <th className="p-2.5 text-[#A1A1AA]">Category</th>
                ...
                <tbody>
                  {data?.best_selling.map((item, idx) => (
                    <tr
                      ...
                      className={`border-b border-[rgba(255,255,255,0.03)] hover:bg-[#6D28D9]/10 cursor-pointer ${selectedCategory?.category === item.category ? "bg-[#6D28D9]/20 font-bold" : ""
                        }`}
                    >
                      <td className="p-2.5 text-white max-w-[200px] truncate">{item.category}</td>
        ...
        <div className="glass-card p-6 flex flex-col justify-between h-[440px]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Top 10 Low Selling Categories</h3>
