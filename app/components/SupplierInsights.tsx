...
import { Search, TrendingUp, TrendingDown, Inbox, ShieldAlert, Award, Package, Percent, FileText } from "lucide-react";
...
export default function SupplierInsights() {
  ...
  const [data, setData] = useState<{ best_selling: SupplierStat[]; lowest_selling: SupplierStat[]; all_suppliers: SupplierStat[] } | null>(null);
  ...
  useEffect(() => {
    async function fetchSupplierData() {
      ...
      try {
        ...
        const res = await fetch(`http://127.0.0.1:8000/api/v1/analytics/suppliers?${queryParams.toString()}`);
        if (res.ok) {
  ...
  useEffect(() => {
    async function fetchTopProducts() {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/v1/analytics/suppliers/top-products?partyname=${encodeURIComponent(selectedSupplier.supplier)}`);
        if (res.ok) {
          const result = await res.json();
  ...
  if (error) {
    return (
      <div className="glass-card p-6 flex flex-col items-center justify-center h-48">
        ...
        <span className="text-sm font-semibold text-[#F4F4F5]">Failed to load supplier analytics data</span>
      </div>
    );
  }
  ...
  const filteredSuppliers = data?.all_suppliers.filter((s) =>
  ...
  return (
    <div className="space-y-6">
          ...
          {searchQuery.trim().length > 0 && (
            <div className="absolute left-0 right-0 mt-1 border border-[rgba(255,255,255,0.15)] rounded-lg max-h-60 overflow-y-auto z-50 shadow-2xl" style={{ backgroundColor: "#18181B" }}>
              {filteredSuppliers.length > 0 ? (
                ...
                <div className="text-xs text-[#A1A1AA] p-4 text-center" style={{ backgroundColor: "#18181B", color: "#A1A1AA" }}>No suppliers matched.</div>
              )}
            </div>
          )}
        </div>
      ...
      {/* Supplier detailed Profile Panel & Metrics (Beautified to Impress the Client) */}
      {selectedSupplier && (
        <div className="grid gap-6" style={{ gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}>
          ...
          <div className="glass-card p-6 flex flex-col justify-between min-h-[350px]">
            ...
            <div className="space-y-3.5 mt-6">
              <div className="flex justify-between border-b border-white/15 pb-2">
                ...
                <span className="text-sm font-bold text-white font-mono">{formatCurrency(selectedSupplier.sales)}</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Gross Profit share</span>
                <span className="text-sm font-bold text-emerald-400 font-mono">{formatCurrency(selectedSupplier.profit)}</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Gross Margin %</span>
                <span className="text-sm font-bold text-purple-300 font-mono">{(selectedSupplier.margin_pct || 0).toFixed(2)}%</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Active SKU count supplied</span>
                <span className="text-sm font-bold text-indigo-300 font-mono">{selectedSupplier.sku_count || 0} Products</span>
              </div>
              <div className="flex justify-between pb-1">
                <span className="text-sm text-[#A1A1AA]">Closing inventory stock</span>
                <span className="text-sm font-bold text-blue-300 font-mono">{selectedSupplier.stock_qty.toLocaleString()} Units</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6 flex flex-col justify-between min-h-[350px]">
            <div>
              ...
              <p className="text-[10px] text-[#A1A1AA] mt-0.5 font-mono">Operations tracking metrics</p>
            </div>
            <div className="space-y-3.5 mt-6">
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Incoming Goods Received</span>
                <span className="text-sm font-bold text-emerald-500 font-mono">{(selectedSupplier.receive_qty || 0).toLocaleString()} Units</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Returned Goods to Supplier</span>
                <span className="text-sm font-bold text-rose-500 font-mono">{(selectedSupplier.return_qty || 0).toLocaleString()} Units</span>
              </div>
              <div className="flex justify-between border-b border-white/15 pb-2">
                <span className="text-sm text-[#A1A1AA]">Supplier Return Rate %</span>
                <span className={`text-sm font-bold font-mono ${selectedSupplier.return_rate_pct > 10 ? 'text-rose-400' : 'text-emerald-400'}`}>
              ...
              <div className="flex justify-between border-b border-white/15 pb-2">
                ...
                <span className="text-sm font-bold text-white font-mono">
                  {formatCurrency((selectedSupplier.sales || 0) / (selectedSupplier.sku_count || 1))}
                </span>
              </div>
              <div className="flex justify-between pb-1">
                <span className="text-sm text-[#A1A1AA]">Margin Grade Status</span>
                <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${selectedSupplier.margin_pct > 12 ? 'bg-emerald-900/40 text-emerald-300 border border-emerald-700/40' : 'bg-amber-900/40 text-amber-300 border border-amber-700/40'
                  }`}>
            ...
            {loadingProducts ? (
              ...
              <div className="flex-1 space-y-3 mt-6">
                {topProducts.map((p, idx) => (
                  <div key={idx} className="flex justify-between items-center text-[11px] font-mono border-b border-white/5 pb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 flex flex-col justify-between h-[440px]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Top 10 Best Selling Suppliers</h3>
              ...
              <table className="w-full text-left text-[11px] font-mono border-collapse">
                <thead>
                  <tr className="border-b border-[rgba(255,255,255,0.08)] bg-white/5 sticky top-0 bg-[#121217]">
                    <th className="p-2.5 text-[#A1A1AA]">Supplier</th>
                ...
                <tbody>
                  {data?.best_selling.map((item, idx) => (
                    <tr
                      ...
                      className={`border-b border-[rgba(255,255,255,0.03)] hover:bg-[#6D28D9]/10 cursor-pointer ${selectedSupplier?.supplier === item.supplier ? "bg-[#6D28D9]/20 font-bold" : ""
                        }`}
                    >
                      <td className="p-2.5 text-white max-w-[200px] truncate">{item.supplier}</td>
        ...
        <div className="glass-card p-6 flex flex-col justify-between h-[440px]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Top 10 Low Selling Suppliers</h3>
